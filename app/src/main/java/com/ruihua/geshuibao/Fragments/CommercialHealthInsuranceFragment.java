 ...
 * 商业健康保险  特殊类型 碎片
 */
public class CommercialHealthInsuranceFragment extends BaseFragment {
    ...
    @BindView(R.id.tv_tax_recognition_code)
    TextView tvTaxRecognitionCode;
    @BindView(R.id.tv_date_of_policy)
    TextView tvDateOfPolicy;
    @BindView(R.id.ev_annual_premium)
    EditText evAnnualPremium;
    @BindView(R.id.ev_monthly_premium)
    EditText evMonthlyPremium;
    @BindView(R.id.ev_amount_deducted_from_this_period)
    EditText evAmountDeductedFromThisPeriod;
    ...
    private int REQUEST_CODE_CHOOSE = 120;
    ...
    protected int setLayoutResourceID() {
        return R.layout.fragment_commercial_health_insurance;
    }
    ...
    protected void setUpData() {
        ...
        pvTime = new TimePickerBuilder(mContext, onTimeSetListener)
                //只显示年月日  三个
    ...
    OnTimeSelectListener onTimeSetListener = new OnTimeSelectListener() {
        ...
        public void onTimeSelect(Date date, View v) {
            tvDateOfPolicy.setText(format.format(date));
        }
    };
    ...
    private void setDetailsData() {
        ...
        tvTaxRecognitionCode.setText(TextUtils.isEmpty((String)mBean.getData().getTaxNumber()) ? "" : mBean.getData().getTaxNumber() + "");//税优识别码
        tvDateOfPolicy.setText(TextUtils.isEmpty((String)mBean.getData().getPolicyEffective()) ? "" : mBean.getData().getPolicyEffective()+"");//保单生效日期
        evAnnualPremium.setText(TextUtils.isEmpty((String)mBean.getData().getPolicyMoney()) ? "" : mBean.getData().getPolicyMoney() + "");//年度保费
        evMonthlyPremium.setText(TextUtils.isEmpty((String)mBean.getData().getMonthMoney()) ? "" : mBean.getData().getMonthMoney() + "");//月度保费
        evAmountDeductedFromThisPeriod.setText(TextUtils.isEmpty((String)(mBean.getData().getDeductionAmount())) ? "" : mBean.getData().getDeductionAmount() + "");//本期扣除金额
...
//              获取选择的图片数据
    ...
    @OnClick({R.id.iv_back, R.id.tv_date_of_policy, R.id.tv_head_right})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_head_right://保存按钮
                saveDate();
                break;
            case R.id.tv_date_of_policy://保单生效日期   日期选择弹出
                pvTime.show();
                break;
        }
    }
    ...
    private void saveDate() {
        ...
        DeductionDetailsUrlData.put("policyEffective ",tvDateOfPolicy.getText().toString().trim());
        DeductionDetailsUrlData.put("policyMoney ",evAnnualPremium.getText().toString().trim());
        DeductionDetailsUrlData.put("monthMoney ",evMonthlyPremium.getText().toString().trim());
        DeductionDetailsUrlData.put("deductionAmount ",evAmountDeductedFromThisPeriod.getText().toString().trim());
    ...
    private void UploadPic() {
        ...
        picPath = mSelected.get(picNum).getCompressPath();
